...
from tent.robustbench.model_zoo.enums import BenchmarkDataset
from tent.robustbench.utils import download_gdrive
...
def _load_corruptions_dataset(
    ...
    corruption_numeric = {corruptions[i]: i for i in range(len(corruptions))}

# 将corruption_list中的元素转换为对应的数字
    corruption_dict = {i: corruptions[i] for i in range(len(corruptions))}
    ...
    corruption_list=torch.tensor(corruption_list)[:n_examples]
    return x_test, y_test,corruption_list,rand_idx,corruption_dict
