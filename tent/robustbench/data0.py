...
def _load_dataset(
    ...
    if n_examples is not None:
        ...
        x_test_tensor = transform_test(torch.tensor(x_test)[:n_examples])
...
def _load_corruptions_dataset(
    ...
    x_test = transform_test(torch.tensor(x_test)[:n_examples])
...
def load_tin200(n_examples, severity=None, data_dir=None, shuffle=False, corruptions=None, transform=None):
    ...
    if corruptions is not None:
        for corruption in corruptions:
            dataset = datasets.ImageFolder(os.path.join(data_dir, 'Tiny-ImageNet-C', corruption, str(severity)),
                                           transform=transform)
    else:
