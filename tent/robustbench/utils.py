...
from tent.robustbench.model_zoo import model_dicts as all_models
...
def download_gdrive(gdrive_id, fname_save):
    ...
    response = session.get(url_base, params={'id': gdrive_id}, stream=True)#,proxies=proxies)
    token = get_confirm_token(response)

    if token:
        params = {'id': gdrive_id, 'confirm': token}
        response = session.get(url_base, params=params, stream=True)#,proxies=proxies)
    url_ = f'{url_base}&id={gdrive_id}'

    gdown.download(url_, fname_save, quiet=False)#,proxy='http://127.0.0.1:8889/')
